...
private let hourlyFormatter: DateFormatter = {
    print("Hourly Formatter Created.")
         ...
         let task = session.dataTask(with: url) { (data, response, error) in
             ...
             do {
                ...
                for index in 0..<result.hourly.data.count {
                    ...
                    let hourWeather = hourlyWeather(hour: hour, hourlyIcon: hourlyIcon, hourlyTemperature: temperature, hourlyPrecipProbability: precipProbability)
                    self.hourlyWeatherData.append(hourWeather)
                }
             } catch {
                 print("JSON Error.")
             }
